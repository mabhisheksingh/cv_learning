...
def apply_transforms(batch: dict) -> dict:
    batch["pixel_values"] = [
        student_processor(img.convert("RGB"), return_tensors="pt")["pixel_values"].squeeze(0)
        for img in batch["image"]
    ]
    batch["teacher_pixel_values"] = [
        teacher_processor(img.convert("RGB"), return_tensors="pt")["pixel_values"].squeeze(0)
        for img in batch["image"]
    ]
    del batch["image"]
    return batch
...
class KnowledgeDistillationTrainer(Trainer):
    def __init__(self, teacher_model: torch.nn.Module, temperature: float, alpha: float, **kwargs):
