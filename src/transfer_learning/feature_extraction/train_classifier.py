...
def load_cat_dog_dataset(data_dir: str):
    ...
    print(
        f"Training set: {len(train_images)} images ({len(train_dogs)} dogs, {len(train_cats)} cats)"
    )
    print(
        f"Test set: {len(test_images)} images ({len(test_dogs)} dogs, {len(test_cats)} cats)"
    )
...
def main():
    ...
    # Load dataset
    data_dir = "./data/raw"
    train_images, train_labels, test_images, test_labels = load_cat_dog_dataset(
        data_dir
    )
